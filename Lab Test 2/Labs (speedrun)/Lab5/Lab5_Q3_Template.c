...
{    
    //Else we go find the mod and append it in.
    int idx = Hash(key, Q3Hash.hSize);
    ListNode * curr = Q3Hash.Table[idx].head;
    while(curr != NULL)
    {
        if(curr->key == key)
            return curr;
...
int HashInsert(HashTable* Q3HashPtr, int key)
{
    ...
    if(Q3HashPtr == NULL || Q3HashPtr->hSize == 0)
        return -1;

    //Else we go find the mod and append it in.
    int idx =Hash(key, Q3HashPtr->hSize);

    //Append a new node here 
    ListNode * newNode = malloc(sizeof(ListNode));
    newNode->key = key;
    newNode->next = NULL;
    //Check if at that index, the table is null
    if(Q3HashPtr->Table[idx].head == NULL)
    {
        Q3HashPtr->Table[idx].head = newNode;
    }
    else
    {
        ListNode * curr = Q3HashPtr->Table[idx].head;
