...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2)
{
	...
	while(cur1 != NULL && cur2 != NULL)
	{
		...
		//Suppose we want to insert, then the indexes are in odd numbers respectively.
		//Store a reference to the initial cur next
		temp = cur1->next;
		//Store another refernece to our other cur2 next
		temp2 = cur2->next;
		//Then set cur1 next to be the cur2 
		cur1->next = cur2;
		//Set this next to be the initial next, slotting it in between initial cur and cur next
		cur2->next = temp;
		//Then update both pointers to their temps
		cur1 = temp;
		cur2 = temp2;
	}

	//Then set the last element of ll2 to be cur2(remainders of those that were not added)
	ll2->head = cur2;
}
