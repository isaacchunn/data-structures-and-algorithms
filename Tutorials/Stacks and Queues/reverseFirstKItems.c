...
void reverseFirstKItems(Queue *q, int k)
{
    ...
    //Queue is now 4,5,6, Stack is now 3,2,1
    //Then we push all the elements from the stack into the queue again
    while(!isEmptyStack(&s))
    {
        enqueue(q,pop(&s));
    }
    //The queue is now 4,5,6,3,2,1, Stack is empty.
    //Then using queue size - k (6-3), which was the remainder of elements we didnt touch, we enqueue on dequeue
    for(i = 0; i < q->ll.size - k; i++)
    {
        enqueue(q,dequeue(q));
    }
    //Given that q size is 6, 6 - k is 3, so we dequeued first 3 elements which was 4,5,6 and put it at the back.
    //The queue is now 3,2,1,4,5,6, the order of the last 3 is not touched as it is first in first out.
