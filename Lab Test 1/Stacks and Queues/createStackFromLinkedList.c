...
/// @brief Creates a stack from linked list 
/// @param ll linked list
/// @param s stack to populate
...
/// @brief Removes the even values from the stack
/// @param s 
void removeEvenValues(Stack *s)
{
    //Variable declaration
    ...
    while(!isEmptyStack(s))
    {
        ...
        if(item % 2 == 1)
        {
            //If odd, we should put it back later, sos tore in the temp stack
            push(&tempStack, item);
        }
    }
    //Then while temp stack not empty
